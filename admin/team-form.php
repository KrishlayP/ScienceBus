...
$team = load_team_data();
$section = $_GET['section'] ?? 'main_team';
$id = $_GET['id'] ?? '';
$member = [];
...
<div class="max-w-2xl">
    ...
    <form id="teamForm" class="admin-surface mt-4 bg-white rounded-2xl border shadow-sm p-6" enctype="multipart/form-data">
        <input name="id" type="hidden" value="<?= e($member['id'] ?? '') ?>">
        <input name="existing_image" type="hidden" value="<?= e($member['image'] ?? '') ?>">
        ...
        <input name="name" required value="<?= e($member['name'] ?? '') ?>" class="w-full rounded-lg border px-3 py-2 mb-3" placeholder="Name">
        <input name="role" required value="<?= e($member['role'] ?? '') ?>" class="w-full rounded-lg border px-3 py-2 mb-3" placeholder="Role">
        <input name="org" value="<?= e($member['org'] ?? '') ?>" class="w-full rounded-lg border px-3 py-2 mb-3" placeholder="Organization">
        <input name="email" value="<?= e($member['email'] ?? '') ?>" class="w-full rounded-lg border px-3 py-2 mb-3" placeholder="Email">
        <input name="contact" value="<?= e($member['contact'] ?? '') ?>" class="w-full rounded-lg border px-3 py-2 mb-3" placeholder="Contact">
