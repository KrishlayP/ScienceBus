...
async function loadNews() {
  setLoading('newsList', 'cards');
  const res = await request('news');
  clearLoading('newsList');
  document.getElementById('newsList').innerHTML = res.data.map(item => `
    <article class="admin-card border rounded-xl overflow-hidden bg-slate-50">
      <img src="../${html(item.image)}" width="360" height="160" loading="lazy" decoding="async" alt="${html(item.title || 'News image')}" class="w-full h-40 object-cover bg-blue-50 transition duration-300 hover:scale-[1.03]">
      <div class="p-3">
